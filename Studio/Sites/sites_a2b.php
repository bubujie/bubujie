...
print_r($dbh2->errorInfo());
	echo   "\n".'<pre>';
	//$query2 = 'INSERT INTO `jos_weblinks` SET catid=:catid, title=:title, url=:url, ordering=:ordering, state=1, id=:id';
	$query2 = 'INSERT INTO `jos_weblinks`(catid, title, url, ordering, state, id) VALUES(:catid, :title, :url, :ordering, :state, :id)';
	echo "\n",$query2,';';
	$smtp = $dbh2->prepare($query2);
...
//insert into tablename set id='id', name='xxxxx';
		//$smtp = bindParam(':catid', $results[$key][1]);
		//$smtp = bindParam(':title', $results[$key][2]);
		//$smtp = bindParam(':url', $results[$key][3]);
		//$smtp = bindParam(':ordering', $results[$key][4]);
		//$smtp = bindParam(':id', $results[$key][0]);
		
		
		$smtp->execute(array(':catid'=>$results[$key][1], ':title'=>$results[$key][2], ':url'=>$results[$key][3], ':ordering'=>$results[$key][4], ':state'=>1, ':id'=>$results[$key][0]));
